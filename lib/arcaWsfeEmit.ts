...
const afipSoapHttpClient = axios.create({
...
function jsonForDebug(v: unknown, maxLen: number): string {
  try {
...
function getKeyLoose(
  ...
  for (const name of candidates) {
    ...
    const hit = Object.keys(o).find(
      (k) => k.replace(/^[^:]+:/, "").toLowerCase() === name.toLowerCase(),
    )
    if (hit) return o[hit]
  }
  return undefined
...
function formatFecaeCabRejection(resultRoot: Record<string, unknown>): string | null {
  const cab = getKeyLoose(resultRoot, "FeCabResp") as
    | Record<string, unknown>
    | undefined
  if (!cab || typeof cab !== "object") return null
  const res = str(getKeyLoose(cab, "Resultado")).toUpperCase()
  if (res !== "R") return null
  const errs = getKeyLoose(resultRoot, "Errors")
  const parts: string[] = []
  if (errs && typeof errs === "object") {
    const raw = (errs as Record<string, unknown>).Err
    ...
    for (const e of list) {
      if (e && typeof e === "object") {
        ...
        const code = er.Code
        const msg = er.Msg
        if (msg != null) parts.push(`[${code}] ${String(msg)}`)
      }
    }
  }
  if (parts.length === 0) {
    return "AFIP rechazó la solicitud (resultado R en cabecera)."
  }
  return `AFIP: ${parts.join(" ")}`
}

function parseFecaeDetOne(node: unknown): Record<string, unknown> | undefined {
  if (!node || typeof node !== "object") return undefined
  const fr = node as Record<string, unknown>
  const inner = fr.FECAEDetResponse ?? getKeyLoose(fr, "FECAEDetResponse")
  if (Array.isArray(inner) && inner.length > 0) {
    return parseFecaeDetOne(inner[0])
  }
  if (inner && typeof inner === "object") {
    return inner as Record<string, unknown>
  }
  if (
    getKeyLoose(fr, "Resultado", "CAE", "CAEFchVto") !== undefined ||
    fr.Resultado != null ||
    fr.CAE != null
  ) {
    return fr
...
function parseFecaeDetFromFeDetResp(feDetRaw: unknown): Record<string, unknown> | undefined {
  if (feDetRaw == null) return undefined
  if (Array.isArray(feDetRaw)) {
    if (feDetRaw.length === 0) return undefined
    return parseFecaeDetOne(feDetRaw[0])
  }
  if (typeof feDetRaw === "object") {
    return parseFecaeDetOne(feDetRaw)
  }
  return undefined
}

function scoreFecaeDetCandidate(o: Record<string, unknown>): number {
  const cantReg = getKeyLoose(o, "CantReg")
  if (cantReg != null) return 0
  const cae = unwrapSoap(getKeyLoose(o, "CAE", "cae"))
  const caef = unwrapSoap(getKeyLoose(o, "CAEFchVto"))
  const res = unwrapSoap(getKeyLoose(o, "Resultado", "resultado"))
  const obs = getKeyLoose(o, "Observaciones", "observaciones")
  const cbteDesde = getKeyLoose(o, "CbteDesde", "cbteDesde")
  let s = 0
  if (cae != null && String(cae).length > 0) s += 100
  if (caef != null && String(caef).length > 0) s += 40
  if (cbteDesde != null) s += 30
  if (res != null && String(res).length > 0) s += 15
  if (obs != null) s += 10
  return s
}

function pickBestFecaeDetFromTree(root: unknown): Record<string, unknown> | undefined {
  const visited = new WeakSet<object>()
  const scored: { r: Record<string, unknown>; score: number }[] = []
  const walk = (node: unknown, depth: number): void => {
    if (depth > 28 || node === null || typeof node !== "object") return
    if (visited.has(node as object)) return
    visited.add(node as object)
    if (!Array.isArray(node)) {
      const o = node as Record<string, unknown>
      const sc = scoreFecaeDetCandidate(o)
      if (sc > 0) scored.push({ r: o, score: sc })
    }
    const vals = Array.isArray(node)
      ? node
...
function pickWeakFecaeDetFromTree(root: unknown): Record<string, unknown> | undefined {
  const visited = new WeakSet<object>()
  ...
  const walk = (node: unknown, depth: number): void => {
    ...
    if (!Array.isArray(node)) {
      const o = node as Record<string, unknown>
      if (getKeyLoose(o, "CantReg") == null) {
        const res = getKeyLoose(o, "Resultado", "resultado")
...
function unwrapFecaeSoapEnvelope(fecaeRes: unknown): Record<string, unknown> {
  let x: unknown = fecaeRes
  for (let i = 0; i < 8; i++) {
    if (!x || typeof x !== "object") break
    const r = x as Record<string, unknown>
    if (r.FECAESolicitarResult && typeof r.FECAESolicitarResult === "object") {
      return r.FECAESolicitarResult as Record<string, unknown>
    }
    if (r.FECAESolicitarResponse && typeof r.FECAESolicitarResponse === "object") {
      const inner = r.FECAESolicitarResponse as Record<string, unknown>
      if (inner.FECAESolicitarResult && typeof inner.FECAESolicitarResult === "object") {
        return inner.FECAESolicitarResult as Record<string, unknown>
      }
      x = inner
      continue
    }
    ...
    if (r.return && typeof r.return === "object") {
      ...
      if (ret.FECAESolicitarResult && typeof ret.FECAESolicitarResult === "object") {
        return ret.FECAESolicitarResult as Record<string, unknown>
      }
      x = ret
      continue
    }
  ...
  return (fecaeRes as Record<string, unknown>) ?? {}
}
...
export type ArcaEmitResult = {
  ...
  requestPayload?: Record<string, unknown>
  responsePayload?: Record<string, unknown>
  /** Respuesta cruda del cliente SOAP (recortada); para diagnóstico en UI, sin consola. */
...
export async function emitArcaFacturaBConsumidorFinal(
  ...
  try {
    ...
    if (
      homologation &&
      /untrusted|AC de confianza|cms\.cert|cert\.untrusted/i.test(msg)
    ) {
      return {
        ok: false,
        error:
          "AFIP homologación rechazó el certificado (no es de confianza en ese entorno). " +
          "Tenés que usar el par .crt / .key generado en el ambiente de homologación; el certificado de producción no sirve para probar contra WSAA/WSFE homo. " +
          `Detalle: ${msg}`,
      }
    }
  ...
  try {
    client = await soap.createClientAsync(wsfeUrl, {
      disableCache: true,
      request: afipSoapHttpClient,
    })
  } catch (e: unknown) {
  ...
  try {
    ...
    const resultRoot = unwrapFecaeSoapEnvelope(fecaeRes)
    ...
    const feDetRaw =
      (resultRoot?.FeDetResp as unknown) ?? getKeyLoose(resultRoot, "FeDetResp")
    let detResp =
      parseFecaeDetFromFeDetResp(feDetRaw) ??
      parseFecaeDetOne(
        resultRoot?.FECAEDetResponse ??
          getKeyLoose(resultRoot, "FECAEDetResponse"),
      ) ??
      pickBestFecaeDetFromTree(fecaeRes) ??
      pickWeakFecaeDetFromTree(fecaeRes)
    if (!detResp) {
      const cabErr = formatFecaeCabRejection(resultRoot)
      if (cabErr) {
        return {
          ok: false,
          error: cabErr,
      ...
      return {
        ...
        requestPayload: requestPayload as Record<string, unknown>,
        responsePayload,
        debugFecaeSoap: jsonForDebug(fecaeRes, 14_000),
      }
    }

    const resultado =
      str(getKeyLoose(detResp, "Resultado", "resultado")).toUpperCase() || ""
    const cae = str(getKeyLoose(detResp, "CAE", "cae"))
    const caeVtoRaw = str(getKeyLoose(detResp, "CAEFchVto"))
    const obs = getKeyLoose(detResp, "Observaciones", "observaciones")
