-- Guardia de stock atómica en el cobro: lock de inventory_on_hand + capas,
-- y consume_layer para no pisar quantity_remaining con un valor stale.
...
DECLARE
  ...
  allowed TEXT[] := ARRAY[
    ...
    'sales', 'sale_payments', 'purchase_payments', 'service_charge_payments',
    'counter_orders',
    'dining_salons', 'dining_tables', 'dining_floor_decors',
    'table_sessions', 'table_reservations',
    'comandas', 'comanda_sends'
  ];
  src TEXT;
  has_pop BOOLEAN;
  stock_guard JSONB;
...
BEGIN
  ...
  stock_guard := p_payload->'stock_guard';
  IF stock_guard IS NOT NULL AND jsonb_typeof(stock_guard->'needs') = 'array'
     AND jsonb_array_length(stock_guard->'needs') > 0 THEN
    loc := NULLIF(stock_guard->>'location_id', '')::UUID;
    IF loc IS NULL THEN
      RETURN jsonb_build_object('ok', false, 'error', 'stock_guard_location');
    END IF;

    INSERT INTO public.inventory_on_hand (
      pop_id, article_id, location_id, quantity
    )
    SELECT
      pop,
      NULLIF(n->>'article_id', '')::UUID,
      loc,
      0
    FROM jsonb_array_elements(stock_guard->'needs') AS n
    WHERE NULLIF(n->>'article_id', '') IS NOT NULL
    ON CONFLICT (pop_id, article_id, location_id) DO NOTHING;
    ...
    FROM (
      ...
      WHERE pop_id = pop
        AND location_id = loc
        AND article_id IN (
          SELECT NULLIF(n->>'article_id', '')::UUID
          FROM jsonb_array_elements(stock_guard->'needs') AS n
          WHERE NULLIF(n->>'article_id', '') IS NOT NULL
        )
      ORDER BY article_id
    ...
    FOR need IN SELECT jsonb_array_elements(stock_guard->'needs')
    LOOP
      article := NULLIF(need->>'article_id', '')::UUID;
      gated := COALESCE((need->>'needed')::NUMERIC, 0);
      IF article IS NULL OR gated <= 0.000001 THEN
        CONTINUE;
      END IF;
      SELECT quantity
        INTO on_hand
      ...
      IF gated > COALESCE(on_hand, 0) + 0.000001 THEN
        shortages := shortages || jsonb_build_array(
          jsonb_build_object(
            'articleId', article,
            'articleName', COALESCE(NULLIF(need->>'article_name', ''), 'Insumo'),
            'sources', COALESCE(need->'sources', '[]'::jsonb),
            'needed', gated,
            'onHand', GREATEST(0, COALESCE(on_hand, 0))
          )
        );
      END IF;
    END LOOP;
  ...
  LOOP
    ...
    ELSIF op->>'op' = 'consume_layer' THEN
      rec_id := NULLIF(op->>'id', '')::UUID;
      consume_qty := COALESCE((op->>'quantity')::NUMERIC, 0);
      IF rec_id IS NULL OR consume_qty <= 0 THEN
        RETURN jsonb_build_object('ok', false, 'error', 'invalid_op');
      END IF;
      UPDATE public.inventory_cost_layers
      SET quantity_remaining = quantity_remaining - consume_qty
      WHERE id = rec_id
        AND pop_id = pop
        AND quantity_remaining >= consume_qty - 0.000001;
      GET DIAGNOSTICS touched = ROW_COUNT;
      IF touched = 0 THEN
        RETURN jsonb_build_object('ok', false, 'error', 'stock_changed');
      END IF;
      IF first_id IS NULL THEN
        first_id := rec_id;
      END IF;
