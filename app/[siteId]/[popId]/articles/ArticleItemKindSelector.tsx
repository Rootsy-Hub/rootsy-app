...
import {
  ...
  ARTICLE_ITEM_KIND_STOCK_LABEL_COMPACT,
...
const ARTICLE_ITEM_KIND_SEGMENT_OPTIONS: RootsFormSegmentOption[] =
  ARTICLE_ITEM_KINDS.map((kind) => {
    const full = ARTICLE_ITEM_KIND_STOCK_LABEL[kind]
    const compact = ARTICLE_ITEM_KIND_STOCK_LABEL_COMPACT[kind]
    return {
      value: kind,
      label:
        compact === full ? (
...
export function ArticleItemKindSelector({
  ...
  return (
    <RootsFormSegmentField
      ...
      groupClassName="[&_button]:whitespace-nowrap"
    />
  )
}
